...
class WasmJsEngine(
    ...
    override var playerImage: CommonImage = WasmJsImage(
        Image.makeFromBitmap(AbstractPlatformAssetService.getTmpImageBitmap().asSkiaBitmap())
    )
    ...
    override suspend fun initialize(gameMap: GameMap, collisionImageAndBytes: ImageAndBytes, player: Player, font: Any) {
        ...
        gameMap.items.forEach { item ->
            ...
            itemImageCache[itemCacheKey(item)] = WasmJsImage(image)
        }
        gameMap.enemies.forEach { enemy ->
            ...
            enemyImageCache[enemyCacheKey(enemy)] = WasmJsImage(image)
        }
        mapItem = gameMap.items.firstOrNull() ?: CommonItem()
        mapItemImage = gameMap.items.firstOrNull()?.let { itemImageCache[itemCacheKey(it)] }
            ?: WasmJsImage(Image.makeFromBitmap(mapItem.imageAndBytes.imageBitmap.asSkiaBitmap()))
        playerImage = WasmJsImage(Image.makeFromBitmap(player.imageAndBytes.imageBitmap.asSkiaBitmap()))
    }
    ...
    private fun drawMapElements(
        ...
        for (element in elements) {
            if (element.state != ElementState.INACTIVE && element.cullingCheck(viewPort)) {
                ...
                drawSprite(
                    canvas,
                    (resolvedElementImage as WasmJsImage).image,
                    element,
                    localX,
                    localY,
                    mapElementPaint,
    ...
    private fun getOrCreateFlippedFrame(image: Image, element: GameElement): Image {
        ...
        return flippedFrameCache[cacheKey]?.let { (it as WasmJsImage).image } ?: run {
            ...
            flippedFrameCache[cacheKey] = WasmJsImage(flippedFrame)
            flippedFrame
        }
    }
